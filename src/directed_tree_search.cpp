#include <cellworld_planner/directed_tree_search.h>
...
namespace cell_world::planner {
    ...
    void Directed_tree_search::record(const cell_world::Model_public_state &state) {
        PERF_SCOPE("Directed_tree_search::record");
    ...
    Directed_tree_search::Directed_tree_search(const planner::Static_data &data, Predator &predator) :
            data(data),
            predator(predator),
            belief_state(data),
    ...
    void Directed_tree_search::reset() {
        belief_state.reset();
        history.clear();
    }

    Move Directed_tree_search::get_best_move_ucb1(const Model_public_state &state) {
        PERF_SCOPE("Directed_tree_search::get_best_move_ucb1");
        PERF_START("Directed_tree_search::get_best_move_ucb1:start");
        belief_state.record_state(state);
        auto &history_step = history.emplace_back(state);
        auto &prey_current_cell = state.agents_state[PREY].cell;
        auto &predator_current_cell = state.agents_state[PREDATOR].cell;
        ...
        PERF_STOP("Directed_tree_search::get_best_move_ucb1:start");
        ...
        PERF_START("Directed_tree_search::get_best_move_ucb1:simulations");
        for (unsigned int s = 0; s < data.simulation_parameters.tree_search_parameters.simulations; s++) {
            ...
            int depth = 1;
            PERF_START("Directed_tree_search::get_best_move_ucb1:episode");
            for (unsigned int t = 0;
                ...
                if (prey_cell == data.goal_cell()) {
                    ...
                    PERF_START("Directed_tree_search::get_best_move_ucb1:capture_test");
                    ...
                    PERF_STOP("Directed_tree_search::get_best_move_ucb1:capture_test");
                    PERF_START("Directed_tree_search::get_best_move_ucb1:episode_not_finished");
                    if (!episode_finished) {
                        PERF_START("Directed_tree_search::get_best_move_ucb1:lppo_reached");
                        ...
                        PERF_STOP("Directed_tree_search::get_best_move_ucb1:lppo_reached");
                        PERF_START("Directed_tree_search::get_best_move_ucb1:set_next_step");
                        ...
                        PERF_STOP("Directed_tree_search::get_best_move_ucb1:set_next_step");
                        PERF_START("Directed_tree_search::get_best_move_ucb1:model_update");
                        model.update();
                        model.update();
                        PERF_STOP("Directed_tree_search::get_best_move_ucb1:model_update");
                    }
                    PERF_STOP("Directed_tree_search::get_best_move_ucb1:episode_not_finished");
                }
                PERF_START("Directed_tree_search::get_best_move_ucb1:reward_calculation");
                value += accum_gamma * (step_value);
                accum_gamma *= data.simulation_parameters.reward.gamma;
                PERF_STOP("Directed_tree_search::get_best_move_ucb1:reward_calculation");
            }
            PERF_STOP("Directed_tree_search::get_best_move_ucb1:episode");
            if (!episode_finished) {
                value += accum_gamma * (data.simulation_parameters.reward.incompleteness);
            }
            root.update_reward(value);
        }
        PERF_STOP("Directed_tree_search::get_best_move_ucb1:simulations");

        PERF_START("Directed_tree_search::get_best_move_ucb1:evaluation");
        ...
        PERF_STOP("Directed_tree_search::get_best_move_ucb1:evaluation");
        return best_move;
    }
}